...
from LJ_environment import Environment
...
target_dist = 5 # [BL]
no_fishes = [5, 10, 15, 20, 25, 30, 35, 40, 45, 50]
test_runs = 5
...
# Standard Tank
arena_list = [elem * 10 for elem in [1780, 1780, 1170]]
arena = np.array(arena_list)
arena_center = arena / 2.0


# STATISTICS
for no_fish in no_fishes:
    for test_run in range(test_runs):
        ...
        for fish_id in range(no_fish):
            ...
            fishes.append(Fish(fish_id, target_dist, dynamics, environment))
        ...
        filename = str(no_fish) + '_' + str(test_run+5) # date_time
