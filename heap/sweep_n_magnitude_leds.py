"""Sweeps n_magnitude_leds across all six formation configs and runs exp_formations for each.

For each (formation, n_magnitude_leds) pair, sets the FORMATION_CONFIG and
N_MAGNITUDE_LEDS env vars (read by fishfood/exp_formations.py and
simulation_noise.py respectively) and runs
    python simulation_noise.py exp_formations
as a subprocess, then records which logfile prefix that run produced so
plot_sweep_n_magnitude_leds.py can find and compare them afterwards.

Usage:
    python sweep_n_magnitude_leds.py
"""
import json
import os
import re
...
FORMATIONS = ['F1S', 'F2S', 'F3S', 'F1B', 'F2B', 'F3B']
N_MAGNITUDE_LEDS_VALUES = [0.01, 0.05, 0.1, 0.2, 0.5]

N_MAGNITUDE_LEDS_VALUES = [0.01, 0.1, 0.5]
EXPERIMENT = 'exp_formations'
...
os.system('python plot_sweep_n_magnitude_leds.py')
