...
# safe_distance, approach_distance, distance, angle, pitch_range = 100, 1000, 200, 90, 0 # F3S
...
class Fish():
    ...
    def move(self, robots, rel_pos, dist, leds, abs_leds, duration):
        ...
        if self.id == 0: # leader
            ...
            self.forward(0.07)
            self.depth_ctrl_psensor(500,0.1) # target depth, dorsal freq
            ...
            if rel_dist <= safe_distance:
                ...
                # magnitude = 0.4
                magnitude = 0.2 + 0.1*rel_dist/approach_distance
                self.home(new_pos, magnitude)
