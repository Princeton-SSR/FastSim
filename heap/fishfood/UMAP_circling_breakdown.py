...
class Fish():
    ...
    def __init__(self, my_id, dynamics, environment):
        ...
        # Behavior specific
        self.target_depth = random.randint(400, 1170-400)


    def run(self, duration, behavior):
        ...
        target_pos, vel = self.move(robots, rel_pos, dist, duration, behavior)
    ...
    def move(self, robots, rel_pos, dist, duration, behavior):
        ...
        if behavior == 'circling':
            self.circling(robots, rel_pos)
            self.depth_ctrl_psensor(self.target_depth)

        elif behavior == 'aggregation':
            center, magnitude = self.lj_force(robots, rel_pos, dist, r_target=300)
            move = center
