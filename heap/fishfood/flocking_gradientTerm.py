...
class Fish():
    ...
    def move(self, robots, rel_pos, dist, duration):
        ...
        if self.id == 0: # leader
            ...
            magnitude = min(np.linalg.norm(u_i)* leader_speed * len(neighbors), 5)  # move magnitude, limit freq to 5 
            # magnitude = min( np.linalg.norm(phi_a_value) * leader_speed * 2, 5) # follower speed is twice the leader speed so that it can catch up with the leader
