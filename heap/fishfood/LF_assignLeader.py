...
class Fish():
    ...
    def run(self, duration):
        ...
        target_pos, vel = self.move(robots, rel_pos, dist, leds, duration)
    ...
    def move(self, robots, rel_pos, dist, leds,  duration):
        ...
        distance = 400 # 200 mm distance to maintain
        angle = -90
        ...
        if self.id == 0: # leader
            ...
            # self.spin(0.2, 0.1, False) # caudal, pect, cw
            self.forward(0.1)
            self.depth_ctrl_psensor(250,0.1) # target depth, dorsal freq

        elif leds.size != 0: # follower and leader can be seen 
            ...
            if self.id % 3 == 0: # follower group 1
                new_pos = self.translate(r_move_g, heading_vector, angle, distance)  #   pos, vector, keep_angle,distance)
            elif self.id % 3 == 1: # follower group 2
                new_pos = self.translate(r_move_g, heading_vector, -angle, distance)  #   pos, vector, keep_angle,distance)
            else: # follower group 3
                new_pos = self.translate(r_move_g, heading_vector, 0, distance)  #   pos, vector, keep_angle,distance)
