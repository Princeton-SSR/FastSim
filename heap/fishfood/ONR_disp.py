...
class Fish():
    ...
    def __init__(self, my_id, dynamics, environment):
        ...
        # Dispersion
        self.time = 0
        self.lower_thresh = 0.6*self.body_length
        self.upper_thresh = 7.6*self.body_length

    def run(self, duration, half_time):
        """(1) Get neighbors from environment, (2) move accordingly, (3) update your state in environment
        """
        self.time += duration
        if self.time > half_time:
            r_target = self.lower_thresh
        else:
            r_target = self.upper_thresh

        robots, rel_pos, dist, leds = self.environment.get_robots(self.id)
        target_pos, vel = self.move(robots, rel_pos, dist, duration, r_target)
    ...
    def home(self, r_move_g, magnitude):
        ...
        freq_c = min(1.5/4 + 1/1000 * magnitude, 0.5) # 0.5 should be some 2 Hz

        # target lost or occluded
        if not r_move_g.size:
            self.pect_r = 0
            self.pect_l = 0
            self.caudal = 0
            return
        ...
        # target to the right
            ...
            freq_l = 0.2 + 0.3 * abs(heading) / 155
            self.pect_l = freq_l
            self.pect_r = 0
        ...
        # target to the left
            ...
            freq_r = 0.2 + 0.3 * abs(heading) / 155
            self.pect_r = freq_r
            self.pect_l = 0
    ...
    def move(self, robots, rel_pos, dist, duration, r_target):
        ...
        center, magnitude = self.lj_force(robots, rel_pos, dist, r_target)
        move = center
