...
TODO: divide the fish into 2 groups, they need to recognize their group and the other one and use difffernt d across groups
"""
from math import *
import numpy as np
import time
...
# ONlY for testing purposes, this is not used in the simulation
import os
import csv
    ...
    def move(self, robots, rel_pos, dist, duration):
        ...
        if self.id == 0: # leader
            ...
            if self.id % 2 == 0:
                ...
                d_a = sigma_norm(d) * np.sqrt(2) # increase distance for odd agents
            
            

            masked_dist = dist.copy()
            masked_dist[self.id] = np.inf # ignore self distance
            ...
            u_i = 1.28 * g_i + 0.4 * c_i + 0.4 * u_nav # final move command
