...
class Fish():
    ...
    def move(self, robots, rel_pos, dist, duration):
        ...
        if self.id == 0: # leader
            ...
            self.caudal = leader_speed * a # a is from flock_fct.py, set the upper limit of phi_a function
            ...
            for agent in neighbors:
                ...
                norm_term = True
                if norm_term:
                    norm = np.linalg.norm(gradient_term)
                    normalized_gradient = gradient_term / norm if norm != 0 else gradient_term  # avoid divide-by-zero
                    norm = np.linalg.norm(consensus_term)
                    normalized_consensus = consensus_term / norm if norm != 0 else consensus_term  # avoid divide-by-zero

                    gradient_term = normalized_gradient/len(neighbors)
                    consensus_term = normalized_consensus/len(neighbors)
            ...
            if len(neighbors) > 0:
                phi_a_value /= len(neighbors)  # Average phi_a value
            else:
                phi_a_value = 0  # No neighbors, no interaction
            ...
            u_i = 0.9 * u_i_alpha + 0.9 * u_nav # interaction command + navigation term
            
            
            move = u_i # move direction command
            magnitude = min(np.linalg.norm(u_i_alpha), 5)  # move magnitude, limit freq to 5 
            magnitude = min( np.linalg.norm(phi_a_value) * leader_speed *5, 5) # follower speed is twice the leader speed so that it can catch up with the leader
            ...
            # # print("n_ij", n_ij(self.environment.pos[self.id,:3], self.environment.pos[agent,:3]))
            # # print("n_ij norm", np.linalg.norm(n_ij(self.environment.pos[self.id,:3], self.environment.pos[agent,:3])))
            print("*** u_i_alpha = gradient +  consensus", u_i_alpha)
