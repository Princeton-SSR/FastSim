...
def animate(i):
    ...
    # Change view angle based on animation progress
    if i < N * N_skip * 0.7:
        ax.view_init(50 - 0.1*i, 45+0.1 * i)
    # elif i < 2 * N * N_skip / 3:
...
ani.save('./animations/{}_animation.mp4'.format(filename), fps=fps, extra_args=['-vcodec', 'libx264'], dpi=600)
# plt.show()