...
# Optional: Check if file exists and create it with header if not
if not os.path.exists(test_filename):
    with open(test_filename, mode="w", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(["id", "phi_a_value"])
...
def log_meta():
    ...
    if not os.path.exists('./logfiles_LF/'):
        # Create the folder
        os.makedirs('logfiles_LF')

    with open('./logfiles_LF/{}_meta.txt'.format(filename), 'w') as f:
...
leader_initial = [0, 0, 0, math.pi/2]
...
# initialize fish positions around the leader_initial
...
## TESTING: fix pos for agent 1, 2
pos[1,:] =  [ 1.70470743e+01, -1.92295897e+03,  4.31890177e+00,  6.16722211e-01]
# pos[2,:] =   [ 4.14830217e+02, -2.01499230e+03,  3.52402835e+00,  7.48981030e-01]
...
while True:
    # print("============================================== in simulation, step - ", steps, " ======================= =======================  ")
...
print('Simulation data got saved in ./logfiles_LF/{}_data.txt,\nand corresponding experimental info in ./logfiles_LF/{}_meta.txt.\n -'.format(filename, filename))
print('Create corresponding animation by running >python animation_LF.py {}'.format(filename))
